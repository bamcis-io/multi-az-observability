...
export class OperationAvailabilityAndLatencyDashboard extends Construct implements IOperationAvailabilityAndLatencyDashboard {
  ...
  private static createAvailabilityWidgets(props: OperationAvailabilityWidgetProps, title: string) : IWidget[] {
    ...
    for (let i = 0; i < props.availabilityZoneIds.length; i++) {
      ...
      let keyPrefix4: string = AvailabilityAndLatencyMetrics.nextChar(keyPrefix3);
  ...
  private static createLatencyWidgets(props: OperationLatencyWidgetProps, title: string) : IWidget[] {
    ...
    let stats: string[] = props.latencyMetricDetails.graphedSuccessStatistics !== undefined ? props.latencyMetricDetails.graphedSuccessStatistics : ['p99'];
  ...
  private static createNetworkLoadBalancerWidgets(
    ...
    nlbWidgets.push(new GraphWidget({
      height: 8,
      width: 24,
      title: Fn.sub('${AWS::Region} Processed Bytes'),
      region: Fn.sub('${AWS::Region}'),
  ...
  constructor(scope: Construct, id: string, props: OperationAvailabilityAndLatencyDashboardProps) {
    ...
    let lb: CfnLoadBalancer = props.loadBalancer?.node.defaultChild as CfnLoadBalancer;
