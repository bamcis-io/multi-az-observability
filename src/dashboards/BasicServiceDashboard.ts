...
export class BasicServiceDashboard extends Construct {
  ...
  private static createLoadBalancerWidgets(alarms: {[key: string]: IAlarm}, metrics: {[key: string]: IMetric}): IWidget[] {
    let widgets: IWidget[] = [];
  ...
  private static createNatGatewayWidgets(alarms: {[key: string]: IAlarm}, metrics: {[key: string]: IMetric}): IWidget[] {
    let widgets: IWidget[] = [];
  ...
  private static createTopLevelAlarmWidgets(alarms: {[key: string]: IAlarm}): IWidget[] {
    let widgets: IWidget[] = [];
    ...
    Object.keys(alarms).forEach(availabilityZoneId => {
      widgets.push(new AlarmStatusWidget({
  ...
  constructor(scope: Construct, id: string, props: BasicServiceDashboardProps) {
    ...
    if (props.zonalLoadBalancerIsolatedImpactAlarms !== undefined && props.zonalLoadBalancerFaultRateMetrics !== undefined) {
      widgets.push(BasicServiceDashboard.createLoadBalancerWidgets(
    ...
    this.dashboard = new Dashboard(this, 'BasicServiceDashboard', {
      dashboardName: props.serviceName.toLowerCase() + Fn.sub('-service-${AWS::Region}'),
