import { CfnParameter, NestedStack, NestedStackProps, Stack } from 'aws-cdk-lib';
...
export class StackWithDynamicSource extends NestedStack {
  ...
  private static updateBaseParams(scope: IConstruct, props: NestedStackWithDynamicSourceProps): NestedStackProps {
    if (props.assetsBucketsParameterName !== undefined) {
      ...
      if (currentNode !== undefined) {
        let assetsBucket: IConstruct | undefined = (currentNode as Stack).node.tryFindChild(props.assetsBucketsParameterName);

        if (assetsBucket !== undefined && assetsBucket instanceof CfnParameter) {
          ...
          if (props.parameters === undefined) {
            ...
            if (tmp.parameters !== undefined) {
              tmp.parameters[props.assetsBucketsParameterName] = assetsBucket.valueAsString;

              if (props.assetsBucketPrefixParameterName !== undefined) {
                let assetsBucketPrefix: IConstruct | undefined = (currentNode as Stack).node.tryFindChild(props.assetsBucketPrefixParameterName);
                if (assetsBucketPrefix !== undefined && assetsBucketPrefix instanceof CfnParameter) {
                  tmp.parameters[props.assetsBucketPrefixParameterName] = assetsBucketPrefix.valueAsString;
                }
              }
            }
            ...
            props.parameters[props.assetsBucketsParameterName] = assetsBucket.valueAsString;

            if (props.assetsBucketPrefixParameterName !== undefined) {
              let assetsBucketPrefix: IConstruct | undefined = (currentNode as Stack).node.tryFindChild(props.assetsBucketPrefixParameterName);

              if (assetsBucketPrefix !== undefined && assetsBucketPrefix instanceof CfnParameter) {
                props.parameters[props.assetsBucketPrefixParameterName] = assetsBucketPrefix.valueAsString;
              }
            }
          }
        }
